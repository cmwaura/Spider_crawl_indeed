...
from red_scrap.items import RedScrapItem, DescriptionScrapItem
...
class RedDiceSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for sel in response.xpath('//div [@class="serp-result-content"]'):
            ...
            item["title"] = sel.xpath('.//h3/a[@class="dice-btn-link"]/text()').extract()
            clean_data = ' '.join(item["title"]).strip(' \n\t')
            item['title'] = [clean_data]
            item['url'] = sel.xpath('.//h3/a[@class="dice-btn-link"]/@href').extract()
            item["company"] = sel.xpath('.//li [@class="employer"]/a [@class="dice-btn-link"]/text()').extract()
            item['location'] = sel.xpath('.//li [@class="location"]/text()').extract()
